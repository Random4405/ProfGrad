...
<?php 
if ($_SERVER['REQUEST_URI'] == '/services/krovelnye-raboty'){ echo '<p><img src="/images/krov-rab.jpg" border="0" width="400" style="float: right; margin-left: 15px; margin-right: 15px;" alt="финальны1й этап монтажа кровли, укладка битумной черепицы" /></p>
<p>Если Вы ищете информацию по кровельным работам - значит Вы либо строите дом, либо нацелены осуществить реставрацию своей крыши. Рассматривая расценки на кровельные работы, Вы должны в первую очередь понимать, насколько сложный проект кровли Вы хотите разработать, какие материалы для него хотите использовать и какие этапы работы вообще необходимо учитывать.</p>
...
';
	  ...
	  $id = $this->item->id;
	  //echo "id = ".$id;  
	  if (($id == '29') || (($id >= '165')&&($id <= '172')) || (($id >= '174')&&($id <= '176')) || (($id >= '195')&&($id <= '319')) || (($id >= '365')&&($id <= '370')) || (($id >= '424')&&($id <= '439'))) {
	   ?>
	  
                <div class="for_form_button">Получить текстуру</div>
